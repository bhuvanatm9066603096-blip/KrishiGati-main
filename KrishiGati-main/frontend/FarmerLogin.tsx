...
import { ArrowRight, CheckCircle2, FileCheck2, Leaf, LockKeyhole, Mic, ShieldCheck, MapPin } from "lucide-react";
import { registerFarmer, getAllMandis } from "./api";
import { getCopy } from "./i18n";
import type { Mandi } from "./types";

const languages = ["English", "हिंदी", "অসমীয়া", "বাংলা", "बड़ो", "डोगरी", "ગુજરાતી", "ಕನ್ನಡ", "कश्मीरी", "कोंकणी", "मैथिली", "മലയാളം", "മണിപുരി", "मराठी", "नेपाली", "ଓଡ଼ିଆ", "ਪੰਜਾਬੀ", "संस्कृत", "संथाली", "सिन्ધी", "தமிழ்", "తెలుగు", "اردو"];

type Props = { language: string; onLanguageChange: (language: string) => void; onRegistered: (farmer: { id: string; name: string; phone: string; role: "FARMER" | "OFFICIAL"; selectedMandiId?: string }) => void; onVoice: () => void; onNotice: (message: string) => void };
...
export function FarmerLogin({ language, onLanguageChange, onRegistered, onVoice }: Props) {
  ...
  // Load mandis when component mounts or language changes
  useEffect(() => {
    const loadMandis = async () => {
      setLoadingMandis(true);
      try {
        const result = await getAllMandis(13.1007, 77.5963);
        setMandis(result.mandis || []);
        if (result.mandis && result.mandis.length > 0) {
          setSelectedMandi(result.mandis[0].id);
        }
      } catch (err) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    if (!selectedMandi) {
      setError(text.mandi ? `${text.mandi} is required` : "Mandi selection is required");
      return;
    }
    ...
    try {
      ...
      const farmerWithMandi = { ...result.farmer, selectedMandiId: selectedMandi };
      localStorage.setItem("krishigati_farmer", JSON.stringify(farmerWithMandi));
      onRegistered(farmerWithMandi);
