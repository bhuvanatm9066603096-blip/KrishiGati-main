...
export function updateCrowdStatus(mandiId: string, crowdLevel: string, notes?: string, officialId?: string) {
  return request<{ success: boolean; crowdStatus: string }>(`/api/mandis/${mandiId}/crowd`, {
    method: "POST",
    body: JSON.stringify({ crowdLevel, notes, updatedBy: officialId }),
  });
}

export function updateFarmerLocation(tokenId: string, latitude: number, longitude: number) {
  return request<{ success: boolean }>(`/api/tokens/${tokenId}/location`, {
    method: "POST",
    body: JSON.stringify({ latitude, longitude, timestamp: new Date().toISOString() }),
  });
}

export function getCrowdStatus(mandiId: string) {
  return request<{ crowdStatus: string; lastUpdate: string; notes?: string }>(`/api/mandis/${mandiId}/crowd`);
}
