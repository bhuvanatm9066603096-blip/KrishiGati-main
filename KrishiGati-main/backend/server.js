...
app.post("/api/tokens/book", async (req, res) => {
  try {
    const { farmerId, farmerName, farmerPhone, mandiId, cropType, quantityKg, farmerLocation, slotTime } = req.body;
    ...
    // Broadcast to procurement staff at this mandi
    const bookingUpdate = {
      type: "new_booking",
      booking: {
        tokenId: token.id,
        ...
        slotTime: token.slotTime.toISOString(),
        status: token.status,
      },
    };

    io.to(`mandi:${mandiId}`).emit("procurement:update", bookingUpdate);
...
io.on("connection", (socket) => {
  ...
  socket.on("procurement:join", ({ mandiId, officialId }) => {
    socket.join(`procurement:${mandiId}`);
    console.log(`Procurement staff ${officialId} joined mandi ${mandiId}`);
  });

  // Farmer sends location update
  socket.on("location:update", ({ tokenId, latitude, longitude, mandiId }) => {
    const update = {
      type: "location_update",
      tokenId,
      location: { latitude, longitude, timestamp: new Date().toISOString() },
    };
    io.to(`procurement:${mandiId}`).emit("procurement:update", update);
  });

  // Procurement staff updates crowd status
  socket.on("crowd:update", ({ mandiId, crowdLevel, notes, officialId }) => {
    const update = {
      type: "crowd_update",
      mandiId,
      crowdLevel,
    ...
    io.to(`mandi:${mandiId}`).emit("procurement:update", update);
    io.to(`procurement:${mandiId}`).emit("procurement:update", update);
  });
...
// -------------------------------------------------------------------
// Crowd Status Management API
// -------------------------------------------------------------------
app.post("/api/mandis/:mandiId/crowd", async (req, res) => {
  try {
    const { crowdLevel, notes, updatedBy } = req.body;
    const mandiId = req.params.mandiId;

    if (!crowdLevel || !["low", "medium", "high", "very_high"].includes(crowdLevel)) {
      return res.status(400).json({ error: "Invalid crowd level. Must be: low, medium, high, or very_high" });
    }

    const mandi = await prisma.mandiCenter.update({
      where: { id: mandiId },
      data: {
        crowdStatus: crowdLevel,
        lastCrowdUpdateAt: new Date(),
      },
    });
    ...
    res.json({ success: true, crowdStatus: crowdLevel, mandi });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});

app.get("/api/mandis/:mandiId/crowd", async (req, res) => {
  try {
    const mandi = await prisma.mandiCenter.findUnique({
      where: { id: req.params.mandiId },
    });
    ...
    res.json({
      success: true,
      crowdStatus: mandi.crowdStatus || "low",
      lastUpdate: mandi.lastCrowdUpdateAt || new Date(),
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
// -------------------------------------------------------------------
// Farmer Location Tracking API
// -------------------------------------------------------------------
app.post("/api/tokens/:tokenId/location", async (req, res) => {
  try {
    ...
    // Broadcast location update to procurement staff at this mandi
    io.to(`procurement:${token.mandiId}`).emit("farmer:location", {
      tokenId,
      latitude,
      longitude,
